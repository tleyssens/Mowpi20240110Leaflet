...
module.exports = function (stateManager, io) {
    ...
    // io opslaan
    this.io = io;                    // belangrijk voor oude code die socketApi.io gebruikt
   ...
   const { Mower } = require("./bin/config");
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('KnopMes', function () {              //Mesknop bedient in client
        ...
        socket.on('NMEAsource', function (choice) { //Keuzemenu NMEA bron1
            ...
            if (s.GUI.NMEA.choice === "GPS Simulatie") { //simuleer 
                ...
                socket.on('key', nmeaFunc.KeyReceived) //Key received from client
            }else if (s.GUI.NMEA.choice === "GPSudp") { //stuur motoren aan
                SocketApiDebug('socketFunc.KeyReceived = motoren aansturen')
        ...
        socket.on('ContourChoice', function (choice) {
            ...
            if (s.GUI.contour.choice == "Opnemen") {
                ...
                s.GUI.contour.recordColor = (s.GUI.contour.recordColor == "red" ? "" : "red")
                //SocketApiDebug('kleur aanpassen')
            }
            if (s.GUI.contour.choice == "Openen") {
                // const directoryPath = path.join(__dirname, 'public/MissionPlan')
                SocketApiDebug(directoryPath)
                listDir(directoryPath).then((val) => {
            ...
            if (s.GUI.contour.choice == "Opslaan") {
                const data = JSON.stringify(s.GUI.contour.recList);
                fs.writeFile('./public/MissionPlan/Recorded1.json', data, (err) => {
                    if (err) {
                        throw err;
                    }
            ...
            if (s.GUI.contour.choice == "Wissen") {
                s.GUI.contour.recList = []
                //SocketApiDebug(JSON.stringify(s))
            }
            io.sockets.emit('state', s.GUI)
        })
        socket.on('missionBestand', function (naam) { // laad gekozen missiebestand
            fs.readFile(path.resolve(directoryPath, naam), 'utf-8', (err, data) => {
                if (err) {
                    throw err
                }
                // parse JSON object
                ...
                s.GUI.contour.recList = s.mission.data.MissionPlan.MowArea.contour
                SocketApiDebug(s.GUI.contour.recList);
                ////io.sockets.emit('jsonEditor', s.mission.data)
            });
        })
    ...
    async function listDir(path) {
        let names1
        try {
            names1 = fsPromises.readdir(path);
