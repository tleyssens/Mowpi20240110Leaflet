...
exports.update = function (gps) {
  ...
  if (s.contour.isRecordOn) {
    ...
    var contourDriveDistance = GPS.Distance(s.contour.prevPos.lat, s.contour.prevPos.lng, gps.state.lat, gps.state.lon) * 1000;
    console.log(s.contour.prevPos.lat + ',' + s.contour.prevPos.lng + '|' + gps.state.lat + ',' + gps.state.lon);
    console.log('contourDriveDistance' + contourDriveDistance); // timer start
    //console.time('label1');
  ...
  if (s.pos.prev.lat !== null && s.pos.prev.lon !== null) {
    if (s.pos.prev.lat !== s.data.position.pos[0]) {
      ...
      if (positions.length) {
        ...
        if (s.distance1 > s.pos.minDistance) {
      ...
      if (!s.autoMow2) {
        ...
        s.angleError = s.targetHeading - gps.state.bearing; //io.emit('angleError', angleError);
      }
    } else {
      console.log('s.pos.prev.lat gelijk aan s.data.prev.lon');
    }
  ...
  if (s.autoMow2 || s.autoMow3) {
    ...
    if (s.uTurn) {
      ...
      if (Math.abs(s.angleError) < 15) {
        // dit bepaald hoever hij draait na de Uturn. bij 10 gaat hij dikwijls over dezelfde weg terug.
        console.log('hoek OK');
        s.uTurn = false;
      }
    ...
    if (!s.ABLine.set) {
      ...
      console.log('newLat = %s, newLon = %s', s.futurePoint[0], s.futurePoint[1]); //if(contains(s.contour.recList, gps.state.lat, gps.state.lon )) {

      if (contains(s.contour.recList, s.futurePoint[0], s.futurePoint[1])) {
        //s.targetHeading = s.ABLine.heading
        console.log('futurePoint binnen contour => targetHeading = %s', s.targetHeading); //nmeaSimStream.nmea.steerangle = map_range(correction, 255,-255,-90,90)
        ...
        if (s.contour.prevInside) {
          s.uTurn = true;
          s.targetHeading += 180; //s.targetHeading = ((s.targetHeading + 360) % 360) klopt niet
        ...
        if (contains(s.contour.recList, gps.state.lat, gps.state.lon)) {} else {
          ...
          s.GUI.LM = 0;
          s.GUI.RM = 0;
          mower.stop(); //nmeaStream.nmea.stepDistance
...
function calcFuturePoint(lat0, lon0, heading) {
  radiusEarthKilometers = 6371.01; //kmDistance = kmSpeed * (timer1.Interval / 1000f) / 3600f;
  ...
  var endLonRads = startLonRad + Math.Atan2(Math.Sin(angleRadHeading) * distRatioSine * startLatCos, distRatioCosine - startLatSin * Math.Sin(endLatRads));
