...
var debugNMEApandaSimTom = require('debug')('tom:debugNMEApandaSimTom');
...
function () {
  ...
  _createClass(NMEA, [{
    ...
    value: function DoSimTick(_st) {
      ...
      debugNMEApandaSimTom("steerAngle= ".concat(steerAngle, " latNMEA= ").concat(latNMEA, " longNMEA= ").concat(longNMEA, " deg ").concat(degrees, " stepDistance ").concat(this.stepDistance, " temp= ").concat(temp));
...
function CalculateNewPostionFromBearingDistance(lat, lng, bearing, distance) {
  ...
  if (longitude >= 0) EW = 'E';else EW = 'W'; //debugNMEApandaSimTom(latNMEA + "," + longNMEA)
...
var nmea = new NMEA(); //debugNMEApandaSimTom(util.inspect(nmea, true, 3, true))
