...
var missionDebug = require("debug")("tom:mission");

var s = require("../bin/settings.json"),
    Dubins = Promise.resolve().then(function () {
  return _interopRequireWildcard(require("../DubinsJS/dubins.js"));
}),
...
n = 1,
  ...
  n: null
    ...
    targetLon = 0,
    pathSection = [],
    missionPath = []; //let dubWorker = new Dubins()
...
exports.update = function (gps, socketList) {
  ...
  switch (s.mission.data.MissionPlan.State) {
    case 200:
      //Loop over contouren + voeg route naar huis toe
      missionDebug("case 200");
      missionPath.push.apply(missionPath, s.mission.data.MissionPlan.StartRoute); // voeg weg naar Startpunt toe

      traject = s.mission.data.MissionPlan.MowArea.contour;
      Object.values(s.mission.data.MissionPlan.MowList).forEach(function (value) {
        missionDebug(value); //traject = value

        var wp_14 = waypoints(value[0][0], value[0][1], value[3][0], value[3][1]);
        var wp_23 = waypoints(value[1][0], value[1][1], value[2][0], value[2][1]);
        var wp_12 = waypoints(value[0][0], value[0][1], value[1][0], value[1][1]);
        var wp_34 = waypoints(value[2][0], value[2][1], value[3][0], value[3][1]);
      ...
      var newPoints = [];
      var turning_radius = 0.00001; // ~ 1 meter

      var step_size = 0.000003; // ~ .3 meters
      // last point of first segment, assuming array of latLng's
      //let firstSegment = missionPath

      var start = missionPath[0]; // compute heading between second to last and last point

      var startHeading = GPS.Heading(missionPath[0][0], missionPath[0][1], missionPath[1][0], missionPath[1][1]);
      missionDebug("start : %s |  heading : %s", start, startHeading); // first point of last segment, assuming array of latLng's

      var end = missionPath[missionPath.length - 1]; // compute heading between first and second point
      //const endHeading = GPS.Heading(endHeading[0][0], endHeading[0][0], endHeading[1][0], endHeading[1][1])

      var endHeading = GPS.Heading(missionPath[missionPath.length - 2][0], missionPath[missionPath.length - 2][1], missionPath[missionPath.length - 1][0], missionPath[missionPath.length - 1][1]);
      missionDebug("end : %s |  heading : %s", end, endHeading); //const dubWorker = new Dubins()
      //missionDebug(Dubins)
    ...
    case 203:
      // komt hierin door setTimeout
      missionDebug("case 203 missionPath berekenen + dubins?");
      missionDebug(Dubins); //traject = s.mission.data.MissionPlan.MowArea.contour

      traject = missionPath; //s.mission.log += ('||traject103 = ' + traject)//JSON.stringify(traject, null, 4));
    ...
    case 205:
      ...
      socketList.emit("targetA", latlng); //s.mission.log += ('||Huidige positie = ' + latlng)
      ...
      target = GPS.intermediatePoint(prevTrajectLat, prevTrajectLon, trajectLat, trajectLon, minDistance * 16 / trajectDistance); //eerste punt ligt 0,8m verder
    ...
    case 210:
      ...
      if (s.pos.prev.lat !== null && s.pos.prev.lon !== null) {
        if (s.pos.prev.lat !== s.data.position.pos[0] || s.pos.prev.lon !== s.data.position[1]) {
          //was gps.state.lat
          ...
          if (positions.length) {
            ...
            s.nearestPoint = getPerp(prePrevTrajectLat, prePrevTrajectLon, trajectLat, trajectLon, gps.state.lat, gps.state.lon); //missionDebug("gps.state.lat = %s, gps.state.lon = %s", gps.state.lat, gps.state.lon)
          ...
          missionDebug(" info : s.pos.prev.lat gelijk aan s.data.prev.lat");
          break;
        }
      }
      ...
      if (targetDistance < minDistance * 10) {
        ...
        if (trajectDistance < minDistance * 5) {
          ...
          target = GPS.intermediatePoint(prevTrajectLat, prevTrajectLon, trajectLat, trajectLon, minDistance * 3 / trajectDistance); //was 3//8
          ...
          target = GPS.intermediatePoint(prevTrajectLat, prevTrajectLon, trajectLat, trajectLon, minDistance * 4 / trajectDistance); //was 4//10 0.05*4/
      ...
      if (s.targetHeading < -180) s.targetHeading += 360;
      s.angleError = s.targetHeading - gps.state.bearing; //minder schommelen rond rechte lijn?
      // if(s.angleError < 5 && s.angleError > -5) {
      // 	s.angleError = 0
      // }

      if (s.angleError > 180) s.angleError -= 360;
      if (s.angleError < -180) s.angleError += 360; //	missionDebug('gps = ' + JSON.stringify(gps, null, 4));
    ...
    case 215:
      ...
      socketList.emit("targetA", s.mission.data.MissionPlan.MowArea.start);
      s.mission.active = false; //mower.state = "stopped"
...
function timeout(arg) {
  ...
  missionDebug("arg was => ".concat(arg));
...
function waypoints(lat1, lon1, lat2, lon2) {
  var wp = [];
  var ds = 0.5; //is the distance between waypoints which correspond to blade width We use 1 meter as the blade is 110cm.

  var l = geod.InverseLine(lat1, lon1, lat2, lon2); //missionDebug('l = ', l)

  var n = Math.round(Math.ceil(l.s13 / ds)); //missionDebug('n = ', n)

  for (var i = 0; i < n + 1; i++) {
    // in range(n+1):
    if (i === 0) {//missionDebug("distance latitude longitude")
    }

    var _s = Math.min(ds * i, l.s13); //missionDebug('s = ', s)


    var g = l.Position(_s, GeographicLib.Geodesic.STANDARD | GeographicLib.Geodesic.LONG_UNROLL);
    wp.push([g["lat2"], g["lon2"]]); //missionDebug('%s %s %s',g['s12'].toFixed(0), g['lat2'].toFixed(6), g['lon2'].toFixed(6))
    //missionDebug(wp)
  }
...
function path(wp14, wp23) {
  ...
  var count14 = 0,
      ...
      is14 = true,
      pathList = [];
  missionDebug("wp lengths 1-4 : %s, 2-3 : %s , min : %s", wp14.length, wp23.length, Math.min(wp14.length, wp23.length));

  for (var i = 0; i < Math.min(wp14.length, wp23.length) - 1; i++) {
    if (is14) {
      if (count14 != 0) {
        pathList.push(wp14[count14]);
        missionDebug("WP14-%s  %s , %s", count14, wp14[count14][0], wp14[count14][1]);
        count14 += 1;
      }
      ...
      missionDebug("WP23-%s  %s , %s", count23, wp23[count23][0], wp23[count23][1]);
      count23 += 1;
      pathList.push(wp23[count23]);
      missionDebug("WP23-%s  %s , %s", count23, wp23[count23][0], wp23[count23][1]);
      count23 += 1;
...
function circlePath(traject) {
  ...
  var pathList = [];
  var wp_14 = waypoints(traject[0][0], traject[0][1], traject[3][0], traject[3][1]);
  var wp_41 = waypoints(traject[3][0], traject[3][1], traject[0][0], traject[0][1]);
  var wp_23 = waypoints(traject[1][0], traject[1][1], traject[2][0], traject[2][1]);
  var wp_32 = waypoints(traject[2][0], traject[2][1], traject[1][0], traject[1][1]);
  var wp_12 = waypoints(traject[0][0], traject[0][1], traject[1][0], traject[1][1]);
  var wp_34 = waypoints(traject[2][0], traject[2][1], traject[3][0], traject[3][1]);
  var turns = [wp_12.length, wp_23.length, wp_34.length, wp_41.length];
  turns.sort(function (a, b) {
    return a - b;
  });
  missionDebug(turns); //missionDebug('12_lengte =%s, 23_length=%s, 34_lengte=%s, 41_length=%s',wp_12.length, wp_23.length, wp_34.length, wp_41.length)
  //for (let i=0; i<Math.floor((Math.max(wp_12.length, wp_23.length, wp_34.length, wp_41.length)/4+1)); i++) {

  for (var i = 0; i < turns[1] / 2; i++) {
    pathList.push(wp_14[0]);
    pathList.push(wp_23[0]);
    pathList.push(wp_32[0]);
    pathList.push(wp_41[0]);
    pathList.push(wp_14[1]);
    var wp_56 = waypoints(wp_14[1][0], wp_14[1][1], wp_23[1][0], wp_23[1][1]);
    var wp_65 = waypoints(wp_23[1][0], wp_23[1][1], wp_14[1][0], wp_14[1][1]);
    var wp_78 = waypoints(wp_32[1][0], wp_32[1][1], wp_41[1][0], wp_41[1][1]);
    var wp_87 = waypoints(wp_41[1][0], wp_41[1][1], wp_32[1][0], wp_32[1][1]);
...
function destinationPoint(lat, lon, distance, bearing) {
  ...
  var toDegrees = function toDegrees(v) {
...
function getPerp(X1, Y1, X2, Y2, X3, Y3) {
  ...
  ************************************************************************************************/
  var XX = X2 - X1;
  var YY = Y2 - Y1;
  var shortestLength = (XX * (X3 - X1) + YY * (Y3 - Y1)) / (XX * XX + YY * YY);
  var X4 = X1 + XX * shortestLength;
  var Y4 = Y1 + YY * shortestLength;
...
function distanceLineInfinite(px, py, x1, y1, x2, y2) {
  ...
  ************************************************************************************************/
  ...
  var distToLine = ((y2 - y1) * px - (x2 - x1) * py + (x2 * y1 - y2 * x1)) / len;
