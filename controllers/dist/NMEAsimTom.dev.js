...
var debugNMEAsimTom = require('debug')('tom:NMEAsimTom');
...
function () {
  ...
  _createClass(NMEA, [{
    ...
    value: function DoSimTick(_st) {
      ...
      debugNMEAsimTom("48 steerAngle= ".concat(steerAngle, " latNMEA= ").concat(latNMEA, " longNMEA= ").concat(longNMEA, " deg ").concat(degrees, " stepDistance ").concat(this.stepDistance, " temp= ").concat(temp));
      this.latitude = latitude;
      this.longitude = longitude;
      this.speed = Math.round(1.944 * this.stepDistance * 5.0, 1); //console.log('speed : ' + this.speed)

      var sbSendText = BuildGGA(latNMEA, longNMEA, NS, EW, this.fixQuality, this.sats, this.HDOP, this.altitude);
      return sbSendText;
    }
  }, {
    key: "DoSimTickHead",
...
function CalculateNewPostionFromBearingDistance(lat, lng, bearing, distance) {
  ...
  if (longitude >= 0) EW = 'E';else EW = 'W'; //console.log(latNMEA + "," + longNMEA)
...
function BuildGGA(latNMEA, longNMEA, NS, EW, fixQuality, sats, HDOP, altitude) {
  ...
  var sumStr = CalculateChecksum(sbGGA);
...
nmea = new NMEA(); //console.log(util.inspect(nmea, true, 3, true))
