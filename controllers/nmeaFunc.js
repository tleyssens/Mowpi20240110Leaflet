...
const isStateManager = !!(
  stateManager && typeof stateManager.get === "function"
);
...
let nmeaStream, stream;
let choice = "GPS Simulatie";
let isGPSPositionInitialized = false;
...
exports.KeyReceived = function (data) {
  switch (data.Key) {
    case "Steering":
      ...
      debugNmeaFunc(
        "101 steerangle in key %s",
        map_range(X, -250, 250, -90, 90),
      );
...
exports.startStream1 = function (socket, passedS, socketList) {
  ...
  if (choice === "GPSudp") {
    debugNmeaFunc(
      "181 ntrip van flepos naar TeensyGPS opstarten (via udp) => todo getRTCMstream.js opstarten via programma. voorlopig opstarten op Pi4B",
    );
  ...
  gps.on("GGA", function (data) {
    ...
    if (data.quality == "rtk" || data.quality == "rtk-float") {
      ...
      gps.fix = GPS.ConvertWGS84ToLocal(
        //northing en easthing berekenen
        gps.latitude,
        gps.longitude,
        gps.latStart,
        gps.lonStart,
      ...
      if (s.GUI.Autosteer) {
        ...
        nmeaStream.nmea.steerangle =
          s.GUI.mf.guidanceLineSteerAngle * 0.01 * 1.5; // was 0.01*1.5 hogere factor is korter draaien 1.85
        if (choice === "Simulatie") {
      ...
      if (noRTKteller > 5) {
        ...
        socketList.emit(
          "log",
          "geen rtk = stoppen over %s seconden",
          1 - noRTKteller / 5,
        );
      }
      mower.stop();
      ...
      gps.on("NDA", function (data) {
        ...
        if (data.quality == "rtk" || data.quality == "rtk-float") {
          ...
          if (s.GUI.Autosteer) {
            ...
            nmeaStream.nmea.stepDistance = 0.05; //(1,8km/u)
            nmeaStream.nmea.steerangle =
              s.GUI.mf.guidanceLineSteerAngle * 0.01 * 1.5; // was 0.01*1.5
            if (choice === "Simulatie") {
...
function UpdateFixPosition(socketList) {
  ...
  if (!isFirstHeadingSet) {
    ...
    if (stepFixPts[2].isSet == 0) {
      ...
      debugNmeaFunc(
        "706 fixHeading %s°",
        (s.GUI.mf.fixHeading * 180) / Math.PI,
      );
