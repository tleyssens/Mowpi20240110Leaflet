...
const s = config.s;                    // backward compat
...
let startCounter = 0, speedCounter = 0;
let gpsHz = isStateManager ? stateManager.get('gpsHz') : s.gpsHz;
...
exports.startStream1 = function (socket, passedS, socketList) {
  ...
  if (s.GUI.NMEA.choice === "GPS") {
    ...
    // } else {
    stream.on('data', line => debugNmeaFunc(`${line}`))
  ...
  if (s.GUI.NMEA.choice === "GPSudp") {
    ...
    const  udpClientGPS = dgram.createSocket("udp4")
    //getRTCMstream nog op te starten
  ...
  if (s.GUI.NMEA.choice === "GPS Simulatie") {
    debugNmeaFunc("160 startStream1 GPS Simulatie")
    // zorgt voor de wijzerplaat
    //gps.state.speed = 0.2;
    nmeaStream = NMEAstream.getStream(s.GUI.NMEA.choice);
    //debugNmeaFunc(Object.getOwnPropertyNames(nmeaStream)) //OK
    stream = nmeaStream.pipe(split("\r\n"));
    // debugNmeaFunc('parser :', parser)
    // if (parser = 'alert') {
    //   socket.emit('alert', 'Maak eerst een keuze in de dropdown')
  ...
  if (s.GUI.NMEA.choice === "GPSudp Simulatie") {
    let udpSender = dgram.createSocket('udp4')
...
exports.Reset = function () {
    debugNmeaFunc("Reset called");
     debugNmeaFunc("198 Reset gedrukt")
    //s.mf.isAutoSteerBtnOn = false
    nmeaStream.nmea.latitude = s.start1.lat
    nmeaStream.nmea.longitude = s.start1.lng
    nmeaStream.nmea.headingTrue = s.start1.richting * Math.PI/180
    //nmeaStream.nmea.time = 100
    nmeaStream.resume()
    //s.pos.target = s.start1
    //s.pos.temp = s.start1
...
function startParsing(stream, s, socketList) {
  ...
  gps.on("GGA", function (data) {  //data bevat de huidige positie
    ...
    let verschil = Date.now() - data.time;
    //socketList.emit('log','verschil' + verschil)
    if (data.quality != prevQuality) {
      console.log('quality = ' + data.quality);
      socketList.emit(
        "quality",
        data.quality + " " + data.satellites + " " + data.hdop
      );
    ...
    socketList.emit('log',{text: "GGA data", data: s})
...
function startParsingUDP(udpClientGPS, s, socketList) {
  ...
  udpClientGPS.on("message", async (buffer, info) => {
      ...
      //data = data.slice(0,-2) //als je dit doet => valid = false
      //s.GUI.INFO = data
      ...
      if (data != lastData) {
        //debugNmeaFunc("409 nieuwe data")
          ...
          gps.on("data", function (data) {
            ...
            if (gps.state.speed < 10) { //20231015 gps.state.speed > 0 && 
              //socket.emit('gpsState', gps.state);//data voor wijzerplaat en grafiek in mapsxx.html
              s.GUI.gps = gps;
            }
          });
          gps.on("NDA", function (data) {  //data bevat de huidige positie
            if (data.time <= lastTime) { // filterd data die dubbel of te laat binnenkomt eruit
              //debugNmeaFunc("Nu : " + data.time )
              //debugNmeaFunc("Vorige: " + lastData)
              return
            }
            lastTime = data.time
            debugNmeaFunc("442 NDA data ontvangen %o ", data)
            //debugNmeaFunc("socketList %o", socketList)
            //var t0 = performance.now();
            if (data.age !== null) { 
              socketList.emit(
                "quality",
                data.quality + " " + data.satellites + " " + data.age
              ); //20201024
            } else { 
                socketList.emit(
                  "quality", data.time
            ...
            if (data.quality == "rtk" || data.quality == "rtk-float") {
              ...
              gps.longitude = data.lon;
              gps.latitude = data.lat;
              gps.fix = GPS.ConvertWGS84ToLocal(
                gps.latitude,
                gps.longitude,
                gps.latStart,
                gps.lonStart
              ...
              //debugNmeaFunc("gps %o", gps)
              debugNmeaFunc("521 gps.state %o | %o",gps.state, gps.fix)
...
function UpdateFixPosition(socketList) {
  ...
  if (!isGPSPositionInitialized) {
    InitializeFirstFewGPSPositions(socketList);
    socketList.emit('log', {text: "nmeaFunc:645 InitGPS"})
  }
  ...
  if (!isFirstHeadingSet) {
    ...
    if (stepFixPts[2].isSet == 0) {
      ...
      if (stepFixPts[0].isSet == 0) {
        ...
        socketList.emit('log', { text: "nmeaFunc:664 in first after", data: stepFixPts[0]});
        return;
      }
      ...
      if (stepFixPts[1].isSet == 0) {
        ...
        stepFixPts.unshift(
          new vecFix2Fix(gps.fix.easting, 0, gps.fix.northing, 1)
        ); //voegt nieuw punt vooraan toe
        socketList.emit('log', { text: "nmeaFunc:682 in second", data: stepFixPts});
        return;
      }
      ...
      socketList.emit('log', { text: "nmeaFunc:695 in third", data: stepFixPts});

      gpsHeading = Math.atan2(
        gps.fix.easting - stepFixPts[2].easting,
        gps.fix.northing - stepFixPts[2].northing
      );
      debugNmeaFunc("701 gpsHeading %s", gpsHeading);

      if (gpsHeading < 0) gpsHeading += 2.0 * Math.PI;
      else if (gpsHeading > 2.0 * Math.PI) gpsHeading -= 2.0 * Math.PI;
      s.GUI.mf.fixHeading = gpsHeading;
      debugNmeaFunc("706 fixHeading %s°", (s.GUI.mf.fixHeading * 180) / Math.PI);
  ...
  debugNmeaFunc("716 avg %s, startspeed %s", gps.avgSpeed, gps.startSpeed);
  ...
  if (Math.abs(gps.avgSpeed) > gps.startSpeed) {
    ...
    if (stepFixPts[0].isSet == 0) distanceCurrentStepFix = 0;
    ...
    dist = 0;
    for (let i = 1; i < totalFixSteps; i++) {
      if (stepFixPts[i].isSet == 0) {
        currentStepFix = i - 1;
        break;
      }
      dist += stepFixPts[i - 1].distance;
      //debugNmeaFunc('#dist ', dist)
      currentStepFix = i;
      if (dist > minFixStepDist) break;
    }
    //most recent heading
    let newHeading = Math.atan2(
      gps.fix.easting - stepFixPts[currentStepFix].easting,
      gps.fix.northing - stepFixPts[currentStepFix].northing
    );
    if (newHeading < 0) newHeading += constants.twoPI;
    else if (newHeading >= constants.twoPI) newHeading -= glm.twoPI;
    s.GUI.speedometer.heading = GPS.toDegrees(newHeading)

    //debugNmeaFunc("newHeading", newHeading);
...
function InitializeFirstFewGPSPositions(socketList) {
  socketList.emit('log',{text: "nmeaFunc:784 InitGPS ", data: startCounter});
  if (!isFirstFixPositionSet) {
    ...
    gps.fix = GPS.ConvertWGS84ToLocal(
      ...
      gps.latStart,
      gps.lonStart
    ); // pn.fix.northing, out pn.fix.easting);
    socketList.emit('log',{text: "nmeaFunc:800 InitGPS", data: gps.fix})
    debugNmeaFunc("801 firstFix gps.fix = %s ", gps.fix);
    console.dir("first fix %s, %s", gps.fix.easting, gps.fix.northing)
    //Draw a grid once we know where in the world we are.

    // //most recent fixes
    prevFix.easting = gps.fix.easting;
    prevFix.northing = gps.fix.northing;
    //debugNmeaFunc(prevFix.GetLength())
    //run once and return
    //if (gps.state.processed > 1) isFirstFixPositionSet = true
    //else return;
    isFirstFixPositionSet = true;
    socketList.emit('log', {text: "nmeaFunc:814 FirstFixPositionSet OK"});
    return;
  } else {
    prevFix.easting = gps.fix.easting;
    prevFix.northing = gps.fix.northing;
    console.assert(isGPSPositionInitialized, "%o", { startCounter })
    if (startCounter > 9) {
      isGPSPositionInitialized = true;
      socketList.emit('log', {text: "nmeaFunc:823 isGPSPositionInitialized OK"});
    }
    return;
  }
}
...
function CalculatePositionHeading() {
  ...
  let guidanceLookDist = gps.avgSpeed * 0.277777 * guidanceLookAheadTime; //(Math.max(tool.toolWidth * 0.5, gps.avgSpeed * 0.277777 * guidanceLookAheadTime));
  s.GUI.mf.guidanceLookPos.easting =
    pivotAxlePos.easting + Math.sin(s.GUI.mf.fixHeading) * guidanceLookDist;
  s.GUI.mf.guidanceLookPos.northing =
    pivotAxlePos.northing + Math.cos(s.GUI.mf.fixHeading) * guidanceLookDist;
  debugNmeaFunc(
    "843 guidanceLookDist = %s, lookPosE = %s, lookPosN = %s",
    guidanceLookDist,
    s.GUI.mf.guidanceLookPos.easting,
    s.GUI.mf.guidanceLookPos.northing
  );
...
function runShellScript(filename) {
  var child = execFile(filename, [], (error, stdout, stderr) => {
    debugNmeaFunc(`${filename} uitvoeren`);
    if (error) {
      //throw error;
      debugNmeaFunc(`runShellScripterror ${error}`); // opgelet accent = backtick
      return;
    }
    debugNmeaFunc(`${filename} uitgevoerd`);
  });
}
...
function sp1() {
  if (s.uTurn) {
    ...
    debugNmeaFunc("878 cor = %s", s.correction);
    //   s.GUI.LM = s.correction > 0 ? s.autoMowSpeed : s.autoMowSpeed + s.correction;202010
    //   s.GUI.RM = s.correction > 0 ? s.autoMowSpeed - s.correction: s.autoMowSpeed;
    s.GUI.LM =
      s.correction > 0
        ? s.autoMowSpeed
        : s.autoMowSpeed + (s.autoMowSpeed / 90) * s.correction; //20201115
    s.GUI.RM =
      s.correction > 0
        ? s.autoMowSpeed - (s.autoMowSpeed / 90) * s.correction
        : s.autoMowSpeed;
  ...
  if (s.GUI.NMEA.choice === "Simulatie") {
    ...
    debugNmeaFunc(
      "896 in sp1 angleError = %s ;targetHeading = %s; steerangle = %s",
      s.angleError,
      s.targetHeading,
      s.correction
    );
