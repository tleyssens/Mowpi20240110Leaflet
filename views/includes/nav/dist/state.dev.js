...
var socket = io();
var containerJsonEditor = document.getElementById("jsoneditor");
...
document.getElementById('pause').addEventListener('change', function () {
  config.options.scales.xAxes[0].realtime.pause = this.checked; //window.myChart.update({duration: 0});
...
socket.on('state', function (GUI) {
  ...
  document.getElementById('customSwitchAutoMow').checked = GUI.autoMow2; //document.getElementById('AutoMow').innerHTML = 'Automatisch = ' + GUI.autoMow2
  ...
  document.getElementById('contourOpnemen').style.backgroundColor = GUI.contour.recordColor; //popup files laten zien 
  ...
  try {
    for (var _iterator = GUI.contour.fileList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
socket.on('s', function (GUI) {
  console.log(GUI);
  updateSpeed(parseInt(GUI.speedometer.avgSpeed * 10) / 10, parseInt(GUI.speedometer.heading * 10) / 10); // updateSpeed(parseInt(GUI.mf.avgSpeed * 10)/10, parseInt(GUI.speedometer.heading * 10)/10)
  //updateSpeed(GUI.gps.state.speed, GUI.gps.state.bearing)
  //updateSpeed2(GUI.targetHeading);

  updateSpeed2(parseInt(GUI.speedometer.localHeading * 10) / 10, GUI.mf.guidanceLineDistanceOff, GUI.speedometer.howManyPathsAway); //te = t1 - Date.now();
  //t1 = Date.now();
  ...
  updateAngleIMU(GUI.angleIMU); //editor.set(GUI.contour.recList)
