...
socket.on("state", function (s) {
  ...
  document.getElementById("contourDD").style.backgroundColor =
    s.GUI.contour.recordColor;
  document.getElementById("contourOpnemen").style.backgroundColor =
    s.GUI.contour.recordColor;
  //popup files laten zien
  for (const item of s.contour.fileList) {
    let str =
      '<div class="form-check"><input class="form-check-input" type="radio" id="' +
      item +
      '" name="flexRadioDefault"><label class="form-check-label" for="' +
      item +
      '">' +
      item +
      "</label></div>";
...
socket.on('s', function (s) {
  ...
  updateAngleIMU(s.angleIMU);
  //editor.set(s.contour.recList)
});
