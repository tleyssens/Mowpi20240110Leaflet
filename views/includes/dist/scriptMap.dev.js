...
var trackPath,
    trajectPath = [];
var prevFutureMarker = null;
var prevTargetpointMarker = null;
var prevPpCircle = null;
var ppCircles = null;
var prevGpCircle = null;
...
function initMap() {
  ...
  var mapLocation = [50.996328561666665, 5.388168066666666]; //console.log(s)

  console.log(mapLocation); //var state = {lat: 51.05033956456792, lng: 5.304905256533743}; //PA
  ...
  $("#draw a").click(function (e) {
    //	 e.preventDefault();
    console.log("draws");
    disable(); // google.maps.event.addDomListener(map.getDiv(), 'mousedown', function (e) {
  ...
  socket.on('position', function (pos) {
    ...
    var point = [pos.lat, pos.lon]; //console.log('point', point)

    line.addLatLng(point);
    line.addTo(map); //GROEN waardes kalman //
  });
  ...
  socket.on('ABLijn', function (puntA, puntB) {
    console.log('ABLijn punt A : %s; punt B : %s', puntA, puntB); ////let path = lineAB.getPath().getArray()
    ////path.push(new google.maps.LatLng(puntA[0],puntA[1]))
    ////path.push(new google.maps.LatLng(puntB[0],puntB[1]))
    ////lineAB.setPath(path)

    var latlngs = [[puntA[0], puntA[1]], [puntB[0], puntB[1]]]; //let lineAB = 

    L.polyline(latlngs, {
      color: 'blue',
      weight: 3
    }).addTo(map); //lineAB.addLatLng([puntA[0],puntA[1]])
    //lineAB.addLatLng([puntB[0],puntB[1]])
    //lineAB.addTo(map)
  });
  socket.on('missionPath', function (input) {
    //20240114 OK
...
function placeTargetpoint(position, map, title) {
  ...
  var image = L.icon({
    iconUrl: 'trajectPoint.png',
    // rood cirkeltje 7px * 7px
  ...
  marker.addTo(map); //console.log(markers)
...
function placeHeading(position, map, title, angle) {// var marker = new google.maps.Marker({
  //     position: position,
  //     map: map,
  //     title: title,
...
setInterval(function () {
  if (down) {
    ...
    outputEl.innerHTML = '<b>Result:</b> ' + ' dx:' + joystick.deltaX().toFixed(1) + ' dy:' + -joystick.deltaY().toFixed(1) + (joystick.right() ? ' right' : '') + (joystick.up() ? ' up' : '') + (joystick.left() ? ' left' : '') + (joystick.down() ? ' down' : '');
    socket.emit('key', {
      Key: 'Steering',
      speed: -joystick.deltaY().toFixed(1) * 2,
      steer: joystick.deltaX().toFixed(1) * 2
