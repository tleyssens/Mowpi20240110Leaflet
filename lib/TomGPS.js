const TomGPS = require("gps");
//uitbreiding van gps.js
let debugTomGPS = require("debug")("tom3:TomGPS");

let mPerDegreeLat = 0;
let mPerDegreeLon = 0;
var R2D = 180 / Math.PI;

//Propertie NDA toevoegen aan bestaande module als kopie van GGA
...
TomGPS.SetLocalMetersPerDegree = function (latStart) {
  ...
  mPerDegreeLat =
    111132.92 -
    559.82 * Math.cos(2.0 * latStart * 0.01745329251994329576923690766743) +
    1.175 * Math.cos(4.0 * latStart * 0.01745329251994329576923690766743) -
    0.0023 * Math.cos(6.0 * latStart * 0.01745329251994329576923690766743);

  mPerDegreeLon =
    111412.84 * Math.cos(latStart * 0.01745329251994329576923690766743) -
    93.5 * Math.cos(3.0 * latStart * 0.01745329251994329576923690766743) +
    0.118 * Math.cos(5.0 * latStart * 0.01745329251994329576923690766743);
  debugTomGPS("mPerDegreeLat, lon : %s,%s", mPerDegreeLat, mPerDegreeLon);
  //ConvertWGS84ToLocal(latitude, longitude);
...
TomGPS.ConvertLocalToWGS84 = function (latStart, lonStart, Easting, Northing) {
  ...
  let Lat = Northing / mPerDegreeLat + latStart;
  mPerDegreeLon =
    111412.84 * Math.cos(Lat * 0.01745329251994329576923690766743) -
    93.5 * Math.cos(3.0 * Lat * 0.01745329251994329576923690766743) +
    0.118 * Math.cos(5.0 * Lat * 0.01745329251994329576923690766743);
  let Lon = Easting / mPerDegreeLon + lonStart;
  return [Lat, Lon];
};
...
TomGPS.ConvertWGS84ToLocal = function (Lat, Lon, latStart, lonStart) {
  ...
  //   latStart
  // );
  let Northing = (Lat - latStart) * mPerDegreeLat;
  let Easting = (Lon - lonStart) * mPerDegreeLon;
  debugTomGPS("63 Northing = %s ;  Easting = %s", Northing, Easting);
...
TomGPS.GetLocalToWSG84_KML = function (Easting, Northing) {
  ...
  return (
    Lon.ToString("N7", CultureInfo.InvariantCulture) +
    "," +
...
TomGPS.DistanceNE = function (pos1, pos2) {
  return Math.sqrt(
    Math.pow(pos1.easting - pos2.easting, 2) +
      Math.pow(pos1.northing - pos2.northing, 2)
  );
};
...
module.exports = TomGPS;

//staat al in gps.js, maar denk dat dat origineel er niet instaat...
// /**
// 		 * Returns the point at given fraction between ‘this’ point and specified point.
// 		 *				https://www.movable-type.co.uk/scripts/latlong.html
// 		 * @param   {LatLon} point - Latitude/longitude of destination point.
