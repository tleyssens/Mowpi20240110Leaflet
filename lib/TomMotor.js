...
var TomMotor = (function() {
	...
	function update() {
		...
		if (this.ramping) {
			this.interval = setInterval(() => ramp2(this), 20);
		}
	}
	...
	function drive(v) {
		...
		let absV = Math.abs(v);
		let pwmVal = (absV <= this.minSpeed) ? 0 : 
                     (absV > 250 ? this.maxSpeed : 
                     Math.round(this.minSpeed + (this.maxSpeed - this.minSpeed) * (absV / 255)));
	...
	function rampDown(angle) { this.ramp(0, angle); }
	function rampUp(angle)   { this.ramp(255, angle); }
