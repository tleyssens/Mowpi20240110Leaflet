...
var TomMotor = (function() {
	...
	function Setup(enable, cw, ccw, minSpeed, maxSpeed) {		
		console.log('In MotorSetup enable:%s; cw:%s, ccw:%s, minSpeed:%s, maxSpeed:%s', enable, cw, ccw, minSpeed, maxSpeed);
		this.enable = enable;
		this.speed = 0;
	...
	function ramp(value, angle) {
		...
		this.accAngle = angle; 
		this.rampDir = 1;
		//console.log('in TomMotor.Ramp '+ ' ' + this.targetSpeed + ' ' + this.accAngle);
		//console.log('this in ramp= ' + JSON.stringify(this));
		if(speedShouldChange(this)) {
				console.log('speedShouldChange');
				this.t1 = 20;
				startRamping(this);
		}
		else {
	...
	function update () {	
		...
		//intervalID = setInterval (function(){
			//console.log('this in update= ' + JSON.stringify(this));
			clearInterval(this.interval);
			if(this.ramping) {
			//console.log('this in update1= ' + JSON.stringify(that));
			ramp2(this);
			}
		//},20);
	}
	...
	function onTargetSpeed () {
		//console.log('in onTargetSpeed');
		if (!this.ramping) {
			if (!this.hitTargetSpeed) {
				console.log('hittargetSpeed = true')
		...
		{
				//console.log('hitTargetSpeed = false')
				return this.hitTargetSpeed = false;
		}
	}
	function rampDown (angle) {
		//console.log	('In rampDown ' + angle);
		this.angle = angle;
		//console.log	('In rampDown ' + this.angle);
	...
	function setSpeed (mot, value) {
		//console.log('\r            mot in setspeed = ' + JSON.stringify(mot) +'\r');
		//process.stdout.write("\r                       mot in setspeed = " + JSON.stringify(mot) + " ");
		//console.log('in setSpeed: value = ' + value + ' targetspeed = ' + mot.targetSpeed);
		...
		if (value > -1 && value < 256) {
			if (mot.speed != value) {
				...
				console.log('    speed in setSpeed = ' + mot.speed);
			...
			//console.log('speed in setSpeed Else = ' + mot.speed);
			mot.motEnable.pwmWrite(mot.speed);
			return mot.speed;
		}
	}
	...
	function drive(v) {
		...
		// console.log("v na map_range = %s" ,v );
	...
	function speedDifference (mot) {
		//console.log('speedDifference = ' + mot.targetSpeed + ' '+ mot.startSpeed +' ' + (mot.targetSpeed - mot.startSpeed));
		mot.rampDir = Math.sign(mot.targetSpeed - mot.startSpeed); 
		return mot.targetSpeed - mot.startSpeed;
	}

	function stopRamping (mot) {
		//console.log('Stop ramping');
		mot.ramping = false;
